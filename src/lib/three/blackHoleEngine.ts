...
export function createBlackHoleEngine(canvas: HTMLCanvasElement): BlackHoleEngine {
	...
	const uniforms = {
		...
		/** Flowing accretion light — camera pose stays fixed */
		uRotSpeed: { value: reducedMotion ? 0.2 : 0.85 }
	};
	...
	const compositePass = new ShaderPass(
		new THREE.ShaderMaterial({
			...
			uniforms: {
				...
				uTime: { value: 0 },
				uVignette: { value: 0.8 },
				uGrain: { value: useBloom ? 0.028 : 0.02 },
				uCA: { value: 0.002 }
			}
		})
	);
	...
	const liveFrameMs = profile.frameMs;
	...
	function frame(now: number) {
		...
		simTime += reducedMotion ? dt * 0.35 : dt;
		updateCamera(dt);

		frameDebt += realDt * 1000;
		if (frameDebt < liveFrameMs * 0.9) return;
	...
	function onVisibility() {
		if (typeof document !== 'undefined' && !document.hidden) startLoop();
	}
